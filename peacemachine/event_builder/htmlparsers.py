...
def cpjorg_story(soup):
    ...
    hold_dict['date'] = str(dateutil.parser.parse(header.find('p').text))
...
def rsforg_story(soup):
    ...
    hold_dict['date'] = str(dateutil.parser.parse(soup.find('span',
...
def amnestyorg_story(soup):
    ...
    hold_dict['date'] = str(dateutil.parser.parse(soup.find('time')['datetime']))
    paras = soup.find('div', {'class':'wysiwyg'}).find_all('p')
    hold_dict['text'] = [p.text.strip() for p in paras]
    return hold_dict
...
def freedomhouseorg_story(soup):
    ...
    hold_dict['date'] = str(dateutil.parser.parse(soup.find('div', {'class':re.compile('field field-name-post-date')}).text))
...
def netblocksorg_story(soup):
    ...
    hold_dict['date'] = str(dateutil.parser.parse(soup.find('time', {'class':'entry-date published'})['datetime']))
...
def reuterscom_story(soup):
    ...
    # date
    try:
        hold_dict['date'] = header.find('div', {
    ...
    if text[0].startswith('(') and text[0].endswith(')'):
        del text[0]
    hold_dict['text'] = text
    hold_dict['text'] = [p for p in hold_dict['text'] if len(p) > 0]
    ...
    for art in articles:
        ...
        for div in divs:
            if div.text == 'AP Photostream':
                continue
            try:
                hold_dict['date'] = str(dateutil.parser.parse(div.text))
                break
            except:
                pass
...
def nytimescom_story(soup):
    ...
    try:
        ...
        hold_dict['text'] = [paragraph.text for paragraph in article_body.find_all('p')]
    except AttributeError:
        article_body = soup.find_all('p', {'class':'story-body-text story-content'})
        hold_dict['text'] = [paragraph.text.strip() for paragraph in article_body]
        hold_dict['text'] = [p for p in hold_dict['text'] if len(p) > 0]
    try:
        # images (in order)
...
def bloombergcom_story(soup):
    ...
    hold_dict['date'] = soup.find('time', attrs={'class':'article-timestamp'}).find('noscript').text.strip()
...
def the_starcoke(soup):
    ...
    try:
        ...
        hold_dict['date'] = str(dateutil.parser.parse(soup.find('div', {'class':'article-published'}).text.strip()))

    try:
        body=soup.find('div', {'class':'field field-name-body'})
        hold_dict['text'] = [paragraph.text.strip() for paragraph in body.find_all('p')][:-1]
        hold_dict['text'] = [p for p in hold_dict['text'] if len(p) > 0]
    except AttributeError:
...
def guardianng_story(soup):
    ...
    hold_dict['date'] = ' '.join(soup.find('div', {'class':re.compile(r'manual-age')}).text.split())
    ...
    try:
        ...
        hold_dict['text'] =  list(filter(None, body.split('\n'))) # getting paragraphs indexed by line breaks; if by 'p', first par is lost. Remove potential empty ones.
        if hold_dict['image_captions']:
            hold_dict['text'][0] = hold_dict['text'][0].replace(hold_dict['image_captions'][0], '') #remove caption match
            hold_dict['text'] = list(filter(None,  hold_dict['text'])) # Remove empty ones. Important to do this again in case all caption was first par  
    except:
        hold_dict['text'] = []
    return(hold_dict)
...
def punchngcom_story(soup):
    ...
    if "Copyright PUNCH." in hold_dict['authors'] or "READ ALSO" in hold_dict['authors']:
        hold_dict['authors'] = []
    hold_dict['date'] = soup.find('time', {'class': re.compile(r'entry-date published')}).text   
    ...
    hold_dict['text'] =  list(filter(None, pars)) # assign pars and remove empty ones
...
def vanguardngrcom_story(soup):
    ...
    if len(pars) is 0: 
        hold_dict['text'] = [soup.find('article').text]
    else:
        for ii in [0,1]:
    ...
    #     hold_dict['text'] = [hold_dict['authors'].split('\n')[1]] + hold_dict['text']   
...
def championnewscomng_story(soup):
    ...
    if pars[0].split(' ')[0].isupper():
        ...
        hold_dict['text'] = [par.split('\n') for par in pars if hold_dict['author'] not in par]
    else:
        hold_dict['author'] = []
        hold_dict['text'] = [par.split('\n') for par in pars]
    if not hold_dict['text']:
        hold_dict['text'] = pars
    hold_dict['text'] = [p.strip() for p in hold_dict['text'] if len(p.strip()) > 0]
    hold_dict['image_urls'] = []
...
def tribuneonlineng_story(soup):
    ...
    hold_dict['text'] = [p.replace('\xa0', '') for p in pars if p is not '']
    hold_dict['text'] = [p for p in hold_dict['text'] if len(p) > 0]
...
def saharareporterscom_story(soup):
    ...
    hold_dict['image_captions'] = [] 
    if 'Saharareporters' in hold_dict['text'][0]:
        hold_dict['text'] = hold_dict['text'][1:len(hold_dict['text'])]
    if 'By' in str(hold_dict['text'][0:2]):     
        pars = hold_dict['text']
        pos = int(np.where(["By" in s for s in pars[0:2]])[0])
        hold_dict['text'] = pars[pos+1:len(pars)]
        hold_dict['authors'] += ''.join(", " + pars[pos][3:len(pars[pos])].encode('ascii', 'ignore').decode('utf-8'))
    hold_dict['text'] = [p.replace(u'\xa0', u' ') for p in hold_dict['text']]
    return(hold_dict)
...
def sunnewsonlinecom_story(soup):
    ...
    hold_dict['text'] = [y for x in pars for y in x] #flatten list of lists
    hold_dict['image_urls'] = []
    try:
    ...
    preps = ['From', 'By', 'BY']
    if any([prep in str(hold_dict['text'][0:4]) for prep in preps]) and len(hold_dict['text'])>=4:       
        pars = hold_dict['text'] # just to make code more legible
    ...
    if 'Source:' in str(hold_dict['text'][0]) and len(hold_dict['text'])>2:
        hold_dict['text'] = hold_dict['text'][1:len(hold_dict['text'])]
    hold_dict['text'] = [p for p in hold_dict['text'] if len(p) > 0]
    return(hold_dict)  
...
def nanng_story(soup):
    ...
    hold_dict['date']=soup.find('time', {'itemprop':'datePublished'})['datetime']
    ...
    if firstline.startswith('By'):
        ...
        hold_dict['text'] = [p.text.encode('ascii', 'ignore').decode() for p in body if not p.has_attr('class')][1:]
    else:
        hold_dict['authors'] = ['']
        hold_dict['text'] = [p.text.encode('ascii', 'ignore').decode() for p in body if not p.has_attr('class')][0:]
    hold_dict['text'] = [p.strip() for p in hold_dict['text'] if len(p.strip()) > 0]
    return hold_dict
...
def dailynewscotz_story(soup):
    ...
    if date:
        ...
        hold_dict['date'] = str(datetime.strptime(date, '%d/%m/%Y'))
    try:
...
def thecitizencotz_story(soup):
        ...
        if bool(soup.find('article', {'class':'main column'})):
            ...
            if bool(header.find('h6')):
                hold_dict['date'] = header.find('h6').text
            else:
                hold_dict['date'] = header.find('h5').text
        else:
            header = soup.find('div', {'class':'heading'})
            hold_dict['title'] = header.find('h1').text
            hold_dict['date'] = str(soup.find('meta', {'property':'article:published_time'})['content'])

        hold_dict['date'] = str(dateutil.parser.parse(hold_dict['date']))
        ...
        if bool(soup.find('section', {'class':'body-copy'})):
            ...
            hold_dict['text'] = [p.text for p in text]
...
def essorml_story(soup):
    ...
    try:
        ...
        hold_dict['date'] = soup.find('li', {'class':'date'}).text.strip()
    ...
    if bool(soup.find('div', attrs={'class':'entry-content clearfix'})):
        ...
        hold_dict['text'] = text[1:]
        hold_dict['text'] = [t for t in hold_dict['text']]
        hold_dict['text'] = [p for p in hold_dict['text'] if len(p) > 0]
        # first_blank = [num for num, tt in enumerate(hold_dict['text']) if len(tt) == 0][0]
        # hold_dict['text'] = hold_dict['text'][:first_blank]
...
def lessorml_story(soup):
    ...
    hold_dict['date'] = mreplace(date, french_months)

    text = soup.find('div', {'class':'article-content clearfix'}).text    

    hold_dict['text'] = [' '.join(text.split())]
