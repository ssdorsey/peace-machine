...
class RSSCrawlCompare(object):
    ...
    def process_item(self, item, spider):
        if spider.name in ['RssCrawler', 'GdeltCrawler']:
            ...
            try:
                old_version = self.db[self.collection].find_one({'url': item['url']})
            ...
            if old_version and (datetime.datetime.strptime(
                    item['download_date'], "%y-%m-%d %H:%M:%S") -
                                            old_version[download_date]) \
...
class MongoStorage(object):
    ...
    def process_item(self, item, spider):
        ...
        # article['html'] = item['spider_response'].body.decode('utf-8')
        article['event_extracted'] = 0

        # try:
        self.db[self.collection].insert_one(article)
