...
__log_pathname_fully_extracted_warcs = "E:/warc_downloads/fullyextractedwarcs.list"
...
def __get_remote_index():
    ...
    """
    # cleanup
    subprocess.getoutput("del tmpaws.txt")
    # get the remote info
    cmd = "aws s3 ls --recursive s3://commoncrawl/crawl-data/CC-NEWS/ --no-sign-request"
    __logger.info('executing: %s', cmd)
    stdout_data = subprocess.getoutput(cmd)

    lines = stdout_data.splitlines()
    lines = [ll.split()[3] for ll in lines if ll.split()[3].startswith('crawl-data')]
    # lines = [ll for ll in lines if '/2019/' in ll or '/2020/' in ll] #TODO: REMOVE!!! RESTRICTS FILES
    return lines
...
def __start_commoncrawl_extractor(warc_download_url, callback_on_article_extracted=None,
    ...
    try:
	    ...
	    print(f'ERROR on WARC {warc_download_url}')
	    pass
