...
import commoncrawl_crawler

############ YOUR CONFIG ############
# download dir for warc files
my_local_download_dir_warc = '/media/spenc/D/warc_downloads'
# download dir for articles
my_local_download_dir_article = 'E:\scraped_sites\cc-news'
# hosts (if None or empty list, any host is OK)
domain_paths = [ii for ii in os.listdir('/home/spenc/Dropbox/peace-machine/peacemachine/data/domains') if ii.startswith('domains_')]
my_filter_valid_hosts = [] 
for dp in domain_paths:
    with open(f'/home/spenc/Dropbox/peace-machine/peacemachine/data/domains/{dp}', 'r') as f:
        my_filter_valid_hosts += f.read().splitlines()
...
my_number_of_extraction_processes = 20
...
def on_valid_article_extracted(article):
    ...
    # save to pymongo
    try:
        _article = article.__dict__
        _article['download_via'] = 'cc-news'
        ml4p['articles'].insert_one(article.__dict__)
    except pymongo.errors.DuplicateKeyError:
        __logger.error('DuplicateKeyError')
        pass
